...
const CourseComponent = ({ objData, onClick }) => {
  ...
  const getCourseTime = (obj) => {
    ...
    arr.map((a) => a.map((b) => times.push(b.time)));
  ...
  const getCourseVideos = (obj) => {
    let result = [];
    for (let child in obj) {
      if (Array.isArray(obj[child])) result.push(obj[child]);
      else if (!Array.isArray(obj[child])) {
        for (let child2 in obj[child]) {
          if (Array.isArray(obj[child][child2])) result.push(obj[child][child2]);
        }
      }
    }
    if (result[0].length > 1) return result[0].length;
  ...
  return (
    <>
      <div className="CourseComponent">
        ...
        >
          <div>
            <img width={'42px'} src={icon} alt="img" />
            <h4 className="bold text-light">&nbsp;&nbsp;{name}</h4>
          </div>
          <div className="course-button">
            <p className="bold text-light">{getCourseVideos(objData)} videos</p>
            <p className="text-light t-14">
              {getCourseTime(objData)} <img width={'12px'} src={chevronIcon} alt="img" />
            </p>
          </div>
        </div>
        {!isBodyHide && (
          <div className="body">
            {videos &&
              videos.map((a, index) => (
                <CourseItem title={a.title} time={a.time} free={a.free} key={index} src={a.src} onClick={onClick} />
              ))}
            {group &&
              group.map((a, index) => <CoursesGroup name={a} objData={playlist[a]} key={index} onClick={onClick} />)}
          </div>
        )}
      </div>
    </>
